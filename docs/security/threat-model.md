...
| **CHK-4** | Assert the only exported component is the app's **single** launcher activity — one `<activity>` with action MAIN **and** category LAUNCHER in one intent-filter. The exemption covers at most one such activity; a second exported launcher exempts *none* (fail closed on ambiguity, since the launcher class name differs from the applicationId by design and cannot be matched by namespace). A deliberate second launcher is a change to this row, not a build edit. | Same task as CHK-1 | A dependency contributes an exported service/receiver/provider/activity-alias, a **second** exported launcher activity (ad/analytics SDKs do this on merge), or an exported activity whose LAUNCHER category lacks the MAIN action |
