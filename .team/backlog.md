...
| D10 | Continuous whole-piece grain on multi-cell tetrominoes. The grain/mottle texture currently tiles per-cell (`particleU/V` is cell-local, `SoftBodyWorld.kt:431-434`) — a deliberate Backend choice to keep grain frequency consistent with the old single-cell block, so a tetromino reads faintly as "four squares" in the material. Body-local UV would make one continuous mass but varies frequency by piece size (needs compensation) and is a data-path change wanting the Architect. Non-blocking: hue + rim/contact-seam carry piece identity; grain is the documented tertiary cue. | Look call best judged on the client's real Fairphone, not SwiftShader. Backend will implement continuous body-space UV on a fresh branch + loop the Architect if wanted after the client sees the real device. | 2026-07-21, UX Designer + Backend Engineer |
