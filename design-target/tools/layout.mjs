...
export function placePieceDiv(cells, hueBase, cellSize, gridPxX0, gridPxY0, opts = {}) {
  // extraStyle is CSS applied to the *wrapping* div only (a transform, for
  // instance) -- it never reaches renderPiece, so it can't be mistaken for
  // a material parameter. A landing squash is exactly this: a transform on
  // the deformable mesh, not a change to what the mesh is made of.
  const { extraStyle = '', ...pieceOpts } = opts;
  const minC = Math.min(...cells.map(c => c[0]));
  const minR = Math.min(...cells.map(c => c[1]));
  const local = cells.map(([c, r]) => [c - minC, r - minR]);
  const { svg, width, height } = renderPiece(local, hueBase, { cellSize, ...pieceOpts });
  ...
  return `<div style="position:absolute;left:${pxX.toFixed(1)}px;top:${pxY.toFixed(1)}px;width:${width.toFixed(1)}px;height:${height.toFixed(1)}px;${extraStyle}">${svg}</div>`;
}
